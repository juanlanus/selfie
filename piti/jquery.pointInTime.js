...
//      });
//
// This displays a control with month, date and year, leaving out the other
// elements (time).
// It is possible to choose any combination in any order, including the most
// ridiculous.  It also tells to show a "/" after the date (instead of the
// default two spaces), to show an empty separator after the year, and that the
// year value must be between 1990 and 2030 inclusive.
// All the properties of the formats object of the defaults can be modified. 
// The control tries to build a JavaScript Date object using the user input and
// the defaults, the validate function returns it as a local date if it's valid
// else it returns false. 
// The absolute min value is the UNIX epoch, Jan 01, 1970 @00:00:00.000Z. If
// only time is displayed then the Date object returned will have a usable
// milliseconds value.
//
...
;(function ( $, window, document, undefined ) {
  ...
  $.Piti = function( theElement, options ) {
    ...
    var defaults = {
      ...
      pickerOptions: {
        popUpHeader: '',
        // a list of either 12 or 24 free texts, the position of each is used
        // to calculate its numeric value in a 24 hours (military style) clock
        ...
        // number of minutes added/subtracted by the mouse wheel or the arrows/+/-
        // keys, normally or holding down the shift key
        minutesPerWheelDelta: 10,
        minutesPerWheelDeltaShifting: 30,
        minutesPerArrowkeyClick: 1,
        minutesPerArrowkeyClickShifting: 5
      }
    };
    ...
    // referenced field, return false if the field is in error
      ...
      if( isFullDateCheck ) {
        if( isOK ) {
          // the date is OK, export and return it (local time)
    ...
    piti.init = function() {
      ...
      // "picker" subfield: create a button to pop the date and time picker
      ...
      // TODO: redundant! - set the id available to the popup
      piti.settings.pickerOptions.piti = piti.id;
...
*/
      ...
      $element.on(
        ...
        function( $e ){
          var $this = $( this );
          // ensure that the focused artifact is one of the piti fields
          if( $( $e.target ).parent().attr( 'id' ) === piti.id ) {
            // is a field: show the spinner arrows
            // TODO: should add a handler to drive the spinner with the mouse wheel
    ...
    piti.displayDateAndTimePicker = function( piti ) {
      // options object values: separate date and time
      var defaultDate = piti.validate();
      if( ! defaultDate ) { defaultDate = new Date(); }
      // separate date and time (locale values)
      var defaultTime = new Date( 1970, 0, 1, defaultDate.getHours(),
      defaultDate.getMinutes(), defaultDate.getSeconds(), defaultDate.getMilliseconds() );
      defaultDate.setMilliseconds( 0 );
      defaultDate.setSeconds( 0 );
      defaultDate.setMinutes( 0 );
      defaultDate.setHours( 0 );
      // initialize selected values
      piti.settings.pickerOptions.selectedDate = defaultDate;
      piti.settings.pickerOptions.selectedTime = defaultTime;

      piti.settings.pickerOptions.defaultDate = defaultDate;
      piti.settings.pickerOptions.defaultTime = defaultTime;

      // ref to the id of the piti fieldset that owns this popup
      piti.settings.pickerOptions.piti = piti.id;

      // disable the piti controls and the calendar button
      var $fs = $( piti.element ).find( 'fieldset' );
      $fs.find( 'input' ).prop( 'disabled', 'disabled' );
      // make the whole fieldset look disabled
      piti.savedFieldsetBGColor = $fs.css( 'backgroundColor' ); 
      $fs.css( 'backgroundColor', $fs.find( 'input' ).css( 'backgroundColor' ) );
      // bring up the popup
      piti.pitiPicker = piti.PitiPicker( piti, piti.settings.pickerOptions );
    ...
    piti.PitiPicker = function( piti, pickerOptions ) {

      var pp = {};
      pp.time = new Date( 0 ); // Thu, 01 Jan 1970 00:00:00 GMT

      pp.config = pickerOptions;
      ...
      // displays the currently selected date and time in in text (feedback)
      // TODO: this won't work nice on IE8
      var showCurrentDateTime = function() {
        var displayTarget = $piti.find( '.piti_pickerDateDisplay' );
        displayTarget.text(
          pp.config.selectedDate.toLocaleString(
            'en-US',
            {
              // weekday: 'short',  deleted: it's redundant
              year: 'numeric',
              month: '2-digit',
              day: '2-digit'
            }
          ) + ' ' +
          pp.config.selectedTime.toLocaleString(
            'en-US',
            {
              // second: '2-digit', deleted to save space, always zeros
              timeZoneName: 'short', // not shown, potentially confusing
              hour: '2-digit',
              minute: '2-digit',
              hour12: true
            }
        ...
        pp.positionTimeCursor( $piti, pp.config.selectedTime );
      };

      // build the popup HTML structure, append to the piti fieldset
      var buildHTML = function() {
        var thePopupHTML = 
        '<div class="piti_pickerDialog">' +
        '  <div class="piti_pickerHeader">' +
        '    <span>' + pp.config.popUpHeader + '<\/span>' +
        '  <\/div>' +
        // THE DATE PICKER
        ...
        // THE TIME PICKER
        ' <div class="piti_timePicker">' +
        '   <div class="piti_timeScale" tabindex="0">' +
        '     <img class="piti_timePointer" src="images/timeCursor.png">' + // TODO: parameterize src
        '   </div>' +
        ...
        '   </ul>  ' +
        ' </div>' + // for .piti_timePicker

        // THE OK AND CANCEL BUTTONS
        '  <div class="piti_buttons">' +
        '    <button type="button" class="piti_doCancel">Cancel<\/button>' +
        '    <button type="button" class="piti_doOK">Done<\/button>' +
        '  <\/div>' +
        '<\/div>'; // for .piti_pickerDialog
        $piti.append(  $( thePopupHTML ) ); // append to the fieldset
        ...
        $piti.find( '.piti_datePicker').datepicker({
          ...
          onSelect: function( theDate, thePicker ) { 
            pp.config.selectedDate = new Date( theDate ); // ISO-8601 format requires IE9+
            showCurrentDateTime();
            thePicker = thePicker; // for JSHint sake
           },
          dateFormat: 'mm/dd/yy', // must be a locale format else the date returned is GMT
          defaultDate: pp.config.defaultDate, // inject the date currently in piti control, if any
          gotoCurrent: true,
          maxDate: '12/31/2030', // TODO: express the top date in piti (pass it in the options)
          minDate: '01/01/1990', // TODO: express the bottom date in piti (pass it in the options)
      ...
      var setDragNDrop = function() {
        ...
        // position the time cursor at a height proportional to the time
        pp.positionTimeCursor = function( $piti, theTime ) {
          // compute the position like the total height of the slider track
          // divided by total milliseconds times the time milliseconds number
          var h = $piti.find( '.piti_allHours' ).height(); // total height in px
          var t = theTime.getTime() / ( 60 * 1000 ); // GMT minute to plot
          t = t - ( theTime.getTimezoneOffset() ); // convert to local time
          $( '.piti_timePointer' ).css( 'top', h / ( 24 * 60 ) * t );
        };

        pp.handleDrag = function( eventTarget, positionTop ) {
          var h = $( eventTarget ).closest( '.piti_timePicker' ).height();
          var newTime = Math.floor( ( 24 * 60 * 60 * 1000 ) / h * positionTop );
          var tzOffset = pp.config.selectedTime.getTimezoneOffset() * 60 * 1000; // TZ offset in ms
          pp.config.selectedTime.setTime( newTime + tzOffset );
          pp.positionTimeCursor( $piti, pp.config.selectedTime );
          showCurrentDateTime();
        };
     
        var h = $piti.find( '.piti_allHours li' ).height(); // TODO: must refer to this piti
        $piti.find( '.piti_timePicker' ).css( 'height', ( ( 12 * h + 1 ) + 'px' ) );   // TODO: use actual tength

        var dragObj = new piti.Dragger(
          '#thePitiDateInput .piti_timePointer',
          { 
            attachElement: null,
            lowerBound: new piti.Position( 60 - 100, -304 ), // 299 - 600 ),  // -304  ...  -114
            upperBound: new piti.Position( 0, -113 ), // -15 ),
      ...
      // enable direct selection of a round hour by clicking on it
      var enableDirectTimeSelection = function() {
        $piti.find( '.piti_allHours li' ).on(
          'click',
          function( $e ) {
            // get the time value from the text of the clicked cell
            var hl = pp.config.hoursList;
            var selectedTime = $( $e.target ).text(); // the visible text
            selectedTime = $.inArray( selectedTime, hl ) * ( hl.length === 24 ? 1 : 2 ); // 0~23*1 or 0~11*2
            pp.config.selectedTime = new Date( 1970, 0, 1, selectedTime ); // time only Date object, local
            showCurrentDateTime();
            pp.positionTimeCursor( $piti, pp.config.selectedTime );
          }
        );
      ...
      var activateButtons = function() {
        $piti.find( '.piti_doOK' ).on(
          'click',
          function( $e ) {
            var selectedPointInTime = new Date(
              piti.settings.pickerOptions.selectedDate.getTime() +
              piti.settings.pickerOptions.selectedTime.getTime() -
              piti.settings.pickerOptions.selectedDate.getTimezoneOffset() * 60 * 1000
            );
            piti.setValue( selectedPointInTime );

            // TODO: encapsulate these next lines into a shared function pp.cleanup()
            // re-enable the piti controls
            $( piti.element ).find( 'input' ).removeProp( 'disabled' );
            $( piti.element ).find( 'fieldset' ).css( 'backgroundColor', piti.savedFieldsetBGColor );
            // remove document-level event handlers
            $( document ).off( 'keydown.' + piti.id );
        ...
        $( document ).on(
          ...
          function( e ) {
            ...
            // TODO: activate the +/- keys to do the same as the up/down arrows
          }
        );
      };

      // change the currently selected time value by the number of minutes
      // in the argument, trimmed to avoid setting a (local) time outside of
      // the 00:00~23:50 bounds
      pp.incrementSelectedTime = function( minutes ) {
        var tzOffset = pp.config.selectedTime.getTimezoneOffset() * 60 * 1000; // TZ offset in ms
        var newTime = pp.config.selectedTime.getTime() + ( minutes * 60 * 1000 ); // selected GMT time
        if( newTime - tzOffset < 0 ) {
          // resulting time < 0: set it to zero
          newTime = tzOffset;
        } else if( ( newTime - tzOffset ) > ( 24 * 60 * 60 * 1000 - 1 ) ) {
          // resulting time > end-of-day: set it to last minute of day
          newTime = ( 24 * 60 * 60 * 1000 - 60000 ) + tzOffset;
        } else {
          // newTime is good
        }
        pp.config.selectedTime.setTime(newTime );
      };

      // enable the mouse wheel response: the mouse wheel deltas change the
      // current time value, then that time is used to reposition the pointer
      var enableMouseWheelTime = function(){
        ...
        pp.addWheelListener = function( elem ) {
          ...
          function mouseWheelHandler( event ) {
            ...
            // appy the delta to the currently selected time
            var mpwd; // the number of minutes per delta with or without shifting
            if( event.shiftKey ) { 
              mpwd = pp.config.minutesPerWheelDeltaShifting;
            } else {
              mpwd = pp.config.minutesPerWheelDelta; 
            }
            if( delta > 0 ) { // decrease time
              pp.incrementSelectedTime( - mpwd * delta );
            } else {
              pp.incrementSelectedTime( - mpwd * delta );
            }
            pp.positionTimeCursor( $piti, pp.config.selectedTime );
      ...
      // enable changing the time with the arrow and +/- keys
      // $piti.find( '.piti_timePointer' ).on(
      $( document ).on(
        'keydown.' + piti.id,
        piti,
        function( $e ){
          ...
          var mpc; // the number of minutes to change with or without shifting
          if( $e.shiftKey ) { 
            mpc = pp.config.minutesPerArrowkeyClickShifting;
          } else {
            mpc = pp.config.minutesPerArrowkeyClick; 
          }

          // check up and down arrows to increment/decrement the time
          if( theKeyCode === 38 ) { // up arrow 38
            pp.incrementSelectedTime( - mpc );
            pp.positionTimeCursor( $piti, pp.config.selectedTime );
            showCurrentDateTime();
            e.preventDefault();
            e.stopPropagation();
    ...
    // options is an object, see defaults below
      ...
      function dragStart( $eventObj ) {
        if( $eventObj.which !== 1 ) { return; } // handle only left mouse button clicks
        if( dragging ) { return; }
        dragging = true;
