...
;(function ( $, window, document, undefined ) {
  ...
  $.Piti = function( theElement, options ) {
    ...
    var defaults = {
      ...
      pickerOptions: {
        popUpHeader: '(Column name and filter reference)',
        // a list of 24 texts, the position of each must be its numeric value
        // in a 24 hours (military style) clock
...
*/
    ...
    piti.PitiPicker = function( piti, pickerOptions ) {
      ...
      var showCurrentDateTime = function() {
        window.console.log( 'defaultDate/Time: ' + pp.config.selectedDate.toLocaleString() + ' ' + pp.config.selectedTime.toLocaleString() );
      ...
      var buildHTML = function() {
        ...
        '    <div  class="piti_timePointer ui-widget-content" tabindex="0"><\/div>' +
      ...
      var setDragNDrop = function() {
        ...
        $piti.find( '.piti_timePicker' ).css( 'height', ( 24 * h + ( hSlider / 2 - h ) + 'px' ) );
        ...
        $piti.find( '.piti_timePointer' ).draggable({
          axis: 'y',
          // grid: [ 1, h ],  no grid
          containment: 'parent',
      ...
      var enableDirectTimeSelection = function() {
        $piti.find( '.piti_allHours li' ).on(
          ...
          function( $e ) {
            ...
            selectedTime = $.inArray( selectedTime, pp.config.hoursList ); // 0~23
