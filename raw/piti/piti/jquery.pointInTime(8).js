...
*/
    ...
    piti.PitiPicker = function( piti, pickerOptions ) {
      ...
      var enableDirectTimeSelection = function() {
        $piti.find( '.piti_allHours li' ).on(
          ...
          function( $e ) {
            ...
            // $( '.piti_allHours li' ).removeClass( 'piti_timeSelected' );         // TODO: DEPRECATED
            // $timeCell.addClass( 'piti_timeSelected' );                           // TODO: DEPRECATED
      ...
      // change the currently selected time by the number of minutes in the
      // argument, discarding any minutes that would set a time out of the
      // 00:00~23:50 bounds
      pp.incrementSelectedTime = function( minutes ) {
        // TODO: must check bounds before applying the delta
        // TODO: must apply the bounded delta to the selectedTime time value
        var timeSelected = pp.config.selectedTime.getTime() / ( 60 * 1000 ) + minutes;
        pp.config.selectedTime.getTimezoneOffset()
        if( timeSelected < 0 ) { timeSelected = 0; }
        if( timeSelected > ( 24 * 60 - 1 ) ) { timeSelected = 24 * 60 - 1; }
      }
