...
*/
    ...
    piti.displayDateAndTimePicker = function( piti ) {
      ...
      window.console.log( 'defaultDate from validate(): ' + defaultDate.toGMTString() );
      if( ! defaultDate ) { defaultDate = new Date(); }
      // separate date and time
      var defaultTime = new Date( Date.UTC( 1970, 0, 1, defaultDate.getUTCHours(),
      defaultDate.getUTCMinutes(), defaultDate.getUTCSeconds(), defaultDate.getUTCMilliseconds() ) );
      defaultDate.setUTCMilliseconds( 0 );
      defaultDate.setUTCSeconds( 0 );
      defaultDate.setUTCMinutes( 0 );
      defaultDate.setUTCHours( 0 );
      // initialize selected values
      window.console.log( 'defaultDate after clearing time: ' + defaultDate.toGMTString() );
      window.console.log( 'defaultTime after clearing time: ' + defaultTime.toGMTString() );
    ...
    piti.PitiPicker = function( piti, pickerOptions ) {
      ...
      // selectedDate and selectedTime
      var showCurrentDateTime = function() {
        var theDate;
        if( pp.config.selectedTime.getTime() ) {
          theDate = new Date( pp.config.selectedDate.getTime() + pp.config.selectedTime.getTime() );
        }
      ...
      var buildHTML = function() {
        ...
        $piti.find( '.piti_datePicker').datepicker({
          ...
          onSelect: function( theDate, thePicker ) { 
            pp.config.selectedDate = new Date( theDate );
            showCurrentDateTime();
          ...
          gotoCurrent: true,
          maxDate: '2045-12-31', // TODO: express the top date in piti (pass it in the options)
          minDate: '1930-01-01', // TODO: express the bottom date in piti (pass it in the options)
      ...
      var setDragNDrop = function() {
        ...
        pp.positionTimeCursor = function( $piti, theTime ) {
          ...
          var a = theTime.getTime() / ( 60 * 1000 ); // GMT minute to plot
          a = a - ( theTime.getTimezoneOffset() ); // convert to local hours
          var p = h / ( 24 * 60 ) * a;
          $( '.piti_timePointer' ).css( 'top', p );
        };
      ...
      var enableDirectTimeSelection = function() {
        $piti.find( '.piti_allHours li' ).on(
          ...
          function( $e ) {
            ...
            pp.config.selectedTime = new Date( 1970, 0, 1, selectedTime ); // time only Date object TODO: local or UTC?
            showCurrentDateTime();
            // reposition the time cursor
            pp.positionTimeCursor( $piti, pp.config.selectedTime );
          }
        );
       };
      ...
      var activateButtons = function() {
        $piti.find( '.piti_doOK' ).on(
          ...
          function( $e ) {
            ...
            // piti.settings.pickerOptions.selectedDate
